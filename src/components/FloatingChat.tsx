 ...
 * - Namen voor developer (Laurens) en klant
 */
      ...
      <AnimatePresence>
        {chatIsOpen && (
          <motion.div
            ...
            className="fixed left-6 bottom-6 z-50 w-[380px] max-w-[calc(100vw-3rem)] h-[500px] max-h-[calc(100vh-6rem)] bg-gray-900 rounded-2xl shadow-2xl border border-white/10 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className={`p-4 border-b border-white/10 flex items-center gap-3 ${
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-white">Chat met Laurens</h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-950">
              {!projectId ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400 text-center px-4">
                  ...
                  <p className="font-medium text-white mb-1">Welkom!</p>
                  <p className="text-sm">Start een project om te kunnen chatten met onze designer.</p>
                </div>
              ) : messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400 text-center">
                  <MessageSquare className="w-10 h-10 mb-3 opacity-50" />
                  <p className="font-medium text-white mb-1">Start het gesprek</p>
                  <p className="text-sm">Stel je vraag en Laurens antwoordt zo snel mogelijk!</p>
                </div>
              ) : (
                messages.map((msg) => (
                  <div
                  ...
                  >
                    <div className={`max-w-[80%] ${msg.from === 'client' ? 'order-2' : ''}`}>
                      ...
                      <div className={`text-xs mb-1 ${
                        msg.from === 'client' ? 'text-right text-emerald-400' : 'text-left text-blue-400'
                      }`}>
                        {msg.from === 'developer' && (
                      ...
                      <div className={`
                        ...
                        ${msg.from === 'client' 
                          ...
                          : 'bg-gray-800 text-white rounded-bl-sm border border-white/10'}
                      `}>
                        {msg.message}
                      </div>
            ...
            {/* Input */}
            {projectId && (
              <form onSubmit={handleSend} className="p-4 border-t border-white/10 bg-gray-900">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-3 rounded-xl bg-gray-800 border border-white/10 text-white placeholder-gray-500 focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
                  />
                  <button
                    type="submit"
