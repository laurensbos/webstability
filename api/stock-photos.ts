...
// Fallback foto's voor als API niet beschikbaar is
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // If no Unsplash API key, return fallback photos
  if (!UNSPLASH_ACCESS_KEY) {
    console.log('[StockPhotos] No Unsplash API key, returning fallback photos')
    return res.status(200).json({
      photos: FALLBACK_PHOTOS.slice(0, photoCount),
      source: 'fallback',
  ...
  try {
    ...
    const searchQuery = searchTerms[0] // Use first/best match
    
    console.log(`[StockPhotos] Searching for: "${searchQuery}" (from query: "${query}")`)
    
    const response = await fetch(
      `https://api.unsplash.com/search/photos?query=${encodeURIComponent(searchQuery)}&per_page=${photoCount}&orientation=landscape`,
      {
        headers: {
